...
def lac_flags(da):
    """
    Mask of valid SCF values (205).
    """
    return (da == 205)

def gac_flags(da):
    """
    Keep only GAC pixels with value 205.
    All other values are replaced with NaN.
    """
    
    da_mask = xr.where(da == 205, da, float("nan"))
    return da_mask
...
def apply_mask(dataset, data_type):
    ...
    """
    if data_type == 'lac':
        ds_masked = dataset.where(dataset.scfv == 205)
