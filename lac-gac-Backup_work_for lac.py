...
def get_area(area='all'):
    ...
    """
    coords_dict = {
        'ch_alps': [46.35, 7.7],                # mountain and glacier
        'ger_blackforest': [48.58, 8.21],       # forest
        'ger_berlin': [52.42, 13.30],           # urban
        'ger_ostfriesland': [53.10, 7.45],      # agriculture and forest
        'swe_central': [61.3, 13.86],           # forest and agriculture
        'nor_westcoast': [60.00, 6.90],         # fjord, mountain and glacier
        'ch_swath': [-70, 100],
    }
    if area == 'all':
        return coords_dict
...
def crop_lac_region(lac, min_lat, min_lon, size=3):
    lat_max = min_lat + 0.05 * size
    lon_max = min_lon + 0.05 * size

    if lac.lat[0] > lac.lat[-1]:  # decreasing latitude
        lat_slice = slice(lat_max, min_lat)
    else:  # increasing latitude
        lat_slice = slice(min_lat, lat_max)

    return lac.sel(
        lon=slice(min_lon, lon_max),
        lat=lat_slice
    )
...
def create_extent(ds, min_latitude, min_longitude, area):
    ...
    max_latitude = min_latitude + area
    max_longitude = min_longitude + area
    ...
    if ds.lon[0] > ds.lon[-1]:
        lon_slice = slice(max_longitude, min_longitude)
    else:
            ...
            for file_path in file_paths:
                ...
                if da_type == 'lac':
                    ...
                    for loc, coords in sel_area.items():
                        ...
                        # Reproject only the cropped LAC region
                        lac_rp = lac_crop

                        # Resample to GAC grid
                        lac_rs = resample_lac(
                            lac_reprojected=lac_crop,
                            gac_template=gac_templ,
                            method='nearest'
                        )
