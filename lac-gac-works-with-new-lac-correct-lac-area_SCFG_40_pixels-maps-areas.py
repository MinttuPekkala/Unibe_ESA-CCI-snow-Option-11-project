...
def apply_mask(dataset, data_type):
    ...
    if data_type == 'lac':
        ds_masked = dataset.where(
            (dataset.scfg >= 0) & (dataset.scfg <= 100)
        )
        #ds_masked = dataset.where(
    ...
    for datatype in os.listdir(data_directory):
        ...
        for year_dir in os.listdir(datatype_dir):
            ...
            if datatype == "lac":
                ...
                subdir = os.path.join(year_path, "scfg")
...
for date, data_types in selected_scenes.items():
            ...
            for file_path in file_paths:
                ...
                if da_type == 'lac':
                    ...
                    for loc, coords in sel_area.items():
                        ...
                        valid = ((lac_crop.scfg >= 0) & (lac_crop.scfg <= 100)).sum().item()
                        ...
                        print("SCFG:", lac_crop.scfg.min().values, lac_crop.scfg.max().values)
                        print(np.unique(lac_crop.scfg.values)[:20])
                        ...
                        print("Crop shape:", lac_crop.scfg.shape)
                        print("Unique values:", np.unique(lac_crop.scfg.values)[:20])
                        ...
                        lac_daily_maps[loc].append(lac_msk.scfg)

                        lac_value = lac_msk.scfg.mean(skipna=True).item()
                ...
                # Process GAG scenes
                    ...
                    for loc, coords in sel_area.items():
                        ...
                        print("SCFG:", gac_ext.scfg.min().values, gac_ext.scfg.max().values)
                        print(np.unique(gac_ext.scfg.values)[:20])
                        ...
                        gac_value = gac_ext.scfg.mean().item()
    ...
    for region, maps in lac_daily_maps.items():
        ...
        lac_out = lac_daily_mean.to_dataset(name="scfg")
    ...
    for region, maps in gac_daily_maps.items():
        ...
        gac_out = gac_daily_mean.to_dataset(name="scfg")
